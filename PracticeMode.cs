...
namespace MatchZy
{
    ...
    public partial class MatchZy
    {
        ...
        // 替代方案：使用一个全局变量来跟踪我们创建的实体
        private List<CBaseModelEntity> spawnMarkerEntities = new List<CBaseModelEntity>();

        // 添加一个字典来保存每个区域标记对应的出生点
        private Dictionary<string, Position> spawnBeamMapping = new Dictionary<string, Position>();
        private const float SPAWN_SQUARE_SIZE = 20.0f; // 正方形大小
        ...
        public void StartPracticeMode()
        {
            ...
            GetSpawns();
            PrintToAllChat($"练习模式已加载！");
            Server.PrintToChatAll($" {ChatColors.Green}出生点: {ChatColors.Default}.spawn, .ctspawn, .tspawn, .bestspawn, .worstspawn");
            Server.PrintToChatAll($" {ChatColors.Green}机器人: {ChatColors.Default}.bot, .kickall, .crouchbot, .boost, .crouchboost, .kick");
            Server.PrintToChatAll($" {ChatColors.Green}投掷物: {ChatColors.Default}.loadnade, .savenade, .importnade, .listnades");
            Server.PrintToChatAll($" {ChatColors.Green}投掷操作: {ChatColors.Default}.rethrow, .throwindex <index>, .lastindex, .delay <number>");
            Server.PrintToChatAll($" {ChatColors.Green}工具与开关: {ChatColors.Default}.clear, .fastforward, .last, .back, .solid, .impacts, .traj");
            Server.PrintToChatAll($" {ChatColors.Green}阵营与其他: {ChatColors.Default}.ct, .t, .spec, .fas, .god, .dryrun, .break, .exitprac");
            Server.PrintToChatAll($" {ChatColors.Default}输入 {ChatColors.Green}.help{ChatColors.Default} 查看完整指令列表");
            ...
            AddTimer(2.5f, () => {
                ...
                // 注册按键绑定
                if (RegisterUseKeyBindings())
                {
                    Server.PrintToChatAll("现在你可以对着出生点方块按E键进行传送！");
                }
            });
        }
        ...
        {
            // 获取原始坐标
            float x = spawn.PlayerPosition.X;
            float y = spawn.PlayerPosition.Y;
            float z = spawn.PlayerPosition.Z;

            // 为这个出生点生成一个唯一标识符
            string spawnId = $"{x}_{y}_{z}";
            
            // 记录这个区域与出生点的映射关系
            spawnBeamMapping[spawnId] = spawn;

            // 计算正方形的四个顶点
            Vector point1 = new Vector(x + SPAWN_SQUARE_SIZE, y + SPAWN_SQUARE_SIZE, z);
            Vector point2 = new Vector(x + SPAWN_SQUARE_SIZE, y - SPAWN_SQUARE_SIZE, z);
            Vector point3 = new Vector(x - SPAWN_SQUARE_SIZE, y + SPAWN_SQUARE_SIZE, z);
            Vector point4 = new Vector(x - SPAWN_SQUARE_SIZE, y - SPAWN_SQUARE_SIZE, z);

            // 创建正方形的四条边
            CreateBeamLine(point1, point2, color, spawnId); // 连接点1和点2
            CreateBeamLine(point2, point4, color, spawnId); // 连接点2和点4
            CreateBeamLine(point4, point3, color, spawnId); // 连接点4和点3
            CreateBeamLine(point3, point1, color, spawnId); // 连接点3和点1
        }

        private void CreateBeamLine(Vector start, Vector end, Color color, string spawnId)
        {
            CBeam? beam = Utilities.CreateEntityByName<CBeam>("beam");
            if (beam == null)
            {
                Log($"Failed to create beam line");
                return;
            }
            ...
            // 记录这个beam实体到我们跟踪的列表中，以便后续清理
            spawnMarkerEntities.Add(beam);
        ...
        public void RemoveSpawnBeams()
        {
            ...
            // 清空映射字典
            spawnBeamMapping.Clear();
        }

        [ConsoleCommand("css_god", "Sets player health to 100 and makes them immune to all damage")]
        public void OnGodCommand(CCSPlayerController? player, CommandInfo? command)
        {
            ...
            // 检查玩家当前的无敌模式状态
            if (godModeEnabled.TryGetValue(userId, out isGodModeEnabled) && isGodModeEnabled)
            {
                // 关闭无敌模式
                godModeEnabled[userId] = false;
                player.PlayerPawn.Value.Health = 100;
                ReplyToUserCommand(player, "无敌模式" + Localizer["matchzy.cc.disabled"]);
            }
            else
            {
                // 开启无敌模式
                godModeEnabled[userId] = true;
                player.PlayerPawn.Value.Health = 100;
                ReplyToUserCommand(player, "无敌模式" + Localizer["matchzy.cc.enabled"]);
            }
        }
        ...
        private void AddBot(CCSPlayerController? player, bool crouch)
        {
            try
            {
                ...
                {
                    // Player was crong while using .bot command
                    crouch = true;
                }
                isSpawningBot = true;
        ...
        public void OnFFCommand(CCSPlayerController? player, CommandInfo? command)
        {
            ...
            Server.PrintToChatAll($"{chatPrefix} 正在快进20秒！");
        ...
        public void OnNoFlashCommand(CCSPlayerController? player, CommandInfo? command) {
            ...
            if (noFlashList.Contains(userId))
            {
                ...
                ReplyToUserCommand(player, "屏蔽闪光已禁用");
            } else {
                noFlashList.Add(userId);
                ReplyToUserCommand(player, "屏蔽闪光已启用，再次使用.noflash禁用");
        ...
        public void OnTimerCommand(CCSPlayerController? player, CommandInfo? command)
        {
            ...
            if (playerTimers.ContainsKey(userId))
            {
                ...
                player.PrintToCenter($"计时器: {timerResult}秒");
                PrintToPlayerChat(player, $"计时器已停止！结果: {timerResult}秒");
            ...
            else
            {
                ...
                PrintToPlayerChat(player, $"计时器已启动！使用!timer停止计时。");
            }
        }
        ...
        public void OnSolidCommand(CCSPlayerController? player, CommandInfo? command)
        {
            ...
            PrintToAllChat($"队友碰撞已设置为 {newSolidValue}");
        }
        ...
        public void OnImpactsCommand(CCSPlayerController? player, CommandInfo? command)
        {
            ...
            PrintToAllChat($"子弹落点已设置为 {newImpactValue}");
        }
        ...
        public void OnTrajCommand(CCSPlayerController? player, CommandInfo? command)
        {
            ...
            PrintToAllChat($"投掷物轨迹已设置为 {!trajValue}");
        }
        ...
        public void OnShowSpawnsCommand(CCSPlayerController? player, CommandInfo? command)
        {
            ...
            if (RegisterUseKeyBindings())
            {
                PrintToPlayerChat(player, "现在你可以对着出生点方块按E键进行传送！");
            }
        }
        ...
        [ConsoleCommand("css_kick", "Kicks the bot that player is currently aiming at")]
        public void OnKickBotCommand(CCSPlayerController? player, CommandInfo? command)
        {
            if (player == null || !IsPlayerValid(player))
            {
                Log("[OnKickBotCommand] 无效的玩家");
                return;
            }
            
            if (!isPractice)
            {
                Log("[OnKickBotCommand] 不在练习模式中");
                ReplyToUserCommand(player, "该命令只能在练习模式下使用");
                return;
            }
            
            Log($"[OnKickBotCommand] 玩家 {player.PlayerName} 尝试踢出准星指向的机器人");
            
            // 检查是否有机器人
            if (pracUsedBots.Count == 0)
            {
                Log("[OnKickBotCommand] 没有可用的机器人");
                ReplyToUserCommand(player, "场上没有机器人");
                return;
            }
            
            CCSPlayerController? targetBot = GetBotPlayerIsAimingAt(player);
            
            if (targetBot != null && targetBot.IsBot && targetBot.UserId.HasValue)
            {
                // 找到了准星指向的机器人，将其踢出
                int botUserId = targetBot.UserId.Value;
                
                Log($"[OnKickBotCommand] 找到机器人 {targetBot.PlayerName}，ID: {botUserId}，准备踢出");
                ...
                ReplyToUserCommand(player, Localizer["matchzy.pm.botremoved", targetBot.PlayerName]);
            }
            else
            {
                // 没有找到准星指向的机器人
                Log("[OnKickBotCommand] 未找到准星指向的机器人");
                ReplyToUserCommand(player, Localizer["matchzy.pm.nobotatcrosshair"]);
            }
        }
        ...
        private CCSPlayerController? GetBotPlayerIsAimingAt(CCSPlayerController player)
        {
            ...
            // 获取玩家的位置和视线方向
            // 使用 AbsOrigin 并加上垂直偏移（约64单位）来模拟眼睛位置
            Vector playerOrigin = player.PlayerPawn.Value.AbsOrigin;
            Vector playerPosition = new Vector(playerOrigin.X, playerOrigin.Y, playerOrigin.Z + 64.0f);
            QAngle playerAngle = player.PlayerPawn.Value.EyeAngles;
            
            // 将角度转换为前方向向量
            Vector forward = QAngleToVector(playerAngle);
            
            // 定义最大射线距离
            float maxDistance = 1000.0f;
            
            // 射线终点
            Vector rayEnd = playerPosition + (forward * maxDistance);
            ...
            // 调试日志
            Log($"[GetBotPlayerIsAimingAt] 玩家位置: {playerPosition}, 角度: {playerAngle}, 前方向: {forward}");
            Log($"[GetBotPlayerIsAimingAt] 机器人数量: {pracUsedBots.Count}");
            
            foreach (var kvp in pracUsedBots)
            {
                if (kvp.Value["controller"] is CCSPlayerController bot && IsPlayerValid(bot) && bot.IsBot)
                {
                    Vector botPosition = bot.PlayerPawn.Value!.AbsOrigin;
                    
                    // 计算机器人到射线的距离
                    float distance = DistanceToRay(botPosition, playerPosition, rayEnd);
                    
                    // 调试日志
                    Log($"[GetBotPlayerIsAimingAt] 机器人 {bot.PlayerName} 位置: {botPosition}, 与射线距离: {distance}");
            ...
            if (closestBot != null)
            {
                Log($"[GetBotPlayerIsAimingAt] 找到最近的机器人: {closestBot.PlayerName}, 距离: {closestDistance}");
            }
            else
            {
                Log($"[GetBotPlayerIsAimingAt] 未找到符合条件的机器人");
            }
            
            return closestBot;
        }
        
        private Vector QAngleToVector(QAngle angle)
        {
            // 将角度转换为弧度
            float pitch = angle.X * (float)Math.PI / 180.0f;
            float yaw = angle.Y * (float)Math.PI / 180.0f;
        ...
        private float DistanceToRay(Vector point, Vector rayStart, Vector rayEnd)
        {
            // 计算射线方向
            Vector rayDir = rayEnd - rayStart;
            float rayLength = rayDir.Length();
            rayDir = rayDir / rayLength; // 将方向标准化
            
            // 计算从起点到点的向量
            Vector toPoint = point - rayStart;
            
            // 点投影到射线上的长度
            float projectionLength = toPoint.X * rayDir.X + toPoint.Y * rayDir.Y + toPoint.Z * rayDir.Z;
            ...
            // 计算投影点
            Vector projectedPoint = rayStart + rayDir * projectionLength;
            
            // 返回点到投影点的距离
            Vector distanceVector = point - projectedPoint;
            return distanceVector.Length();
        }

        // 用一个字典来存储玩家上次按E键的时间，防止过于频繁地检测
        private Dictionary<int, DateTime> lastUseKeyTime = new Dictionary<int, DateTime>();

        // 添加命令来支持E键交互
        [ConsoleCommand("css_usekey_interact", "Interact with spawn markers when looking at them")]
        public void OnUseKeyInteractCommand(CCSPlayerController? player, CommandInfo? command)
        {
            if (!isPractice || !IsPlayerValid(player)) return;
            ...
            // 每次玩家在练习模式中使用.showspawns命令时自动提示
            PrintToPlayerChat(player, "现在你可以对着出生点方块按E键进行传送！");
        }
        
        // 注册按键绑定并添加Tick事件，当showspawns命令被调用时自动调用
        private bool isUseKeyBindingRegistered = false;
        private bool RegisterUseKeyBindings()
        {
            if (isUseKeyBindingRegistered) 
                return false;
            ...
            RegisterListener<Listeners.OnTick>(() => 
            {
                ...
                {
                    if (!IsPlayerValid(player) || player.IsBot || player.IsHLTV)
                        continue;
                    ...
                    {
                        // 添加调试信息，确认按键检测正常工作
                        // Log($"[Debug] 玩家 {player.PlayerName} 按下了E键");
                        
                        // 防止过于频繁地检测，设置1秒的冷却时间
                        if (!lastUseKeyTime.ContainsKey(userId) || 
                            (DateTime.Now - lastUseKeyTime[userId]).TotalSeconds >= 1.0)
                        {
                            lastUseKeyTime[userId] = DateTime.Now;
                            
                            // 检查玩家是否在看着出生点正方形
                            Position? targetSpawn = GetSpawnPlayerIsLookingAt(player);
                            if (targetSpawn != null)
                            {
                                // 在传送前创建一个视觉反馈（例如一个临时光效）
                                CreateTeleportEffect(targetSpawn.PlayerPosition);
                                
                                // 传送玩家到出生点位置
                                targetSpawn.Teleport(player);
                                
                                // 获取当前队伍名称
                                string teamName = player.TeamNum == (byte)CsTeam.CounterTerrorist ? "CT" : "T";
                                
                                // 获取当前出生点在队伍中的索引和总数
                                int spawnIndex = -1;
                                List<Position> teamSpawns = spawnsData[player.TeamNum];
                                
                                // 查找当前出生点在队伍出生点列表中的索引
                                for (int i = 0; i < teamSpawns.Count; i++)
                                {
                                    if (targetSpawn.PlayerPosition.X == teamSpawns[i].PlayerPosition.X &&
                                        targetSpawn.PlayerPosition.Y == teamSpawns[i].PlayerPosition.Y &&
                                        targetSpawn.PlayerPosition.Z == teamSpawns[i].PlayerPosition.Z)
                                    {
                                        spawnIndex = i;
                                        break;
                                    }
                                ...
                                if (spawnIndex != -1)
                                {
                                    ReplyToUserCommand(player, $"已移动至出生点: {spawnIndex + 1}/{teamSpawns.Count}");
                                }
                                else
                                {
                                    // 如果找不到索引，回退到原来的消息
                                    ReplyToUserCommand(player, $"已传送到{teamName}出生点");
                                }
                                
                                // 添加调试信息
                                Log($"[Debug] 玩家 {player.PlayerName} 传送到出生点 {targetSpawn.PlayerPosition.X},{targetSpawn.PlayerPosition.Y},{targetSpawn.PlayerPosition.Z}");
                            }
                            else
                            {
                                // 添加调试日志
                                Log($"[Debug] 玩家 {player.PlayerName} 按下E键，但没有找到目标出生点");
                                ...
                                if (player.PlayerPawn?.Value != null && player.PlayerPawn.Value.CBodyComponent?.SceneNode != null)
                                {
                                    ...
                                    Log($"[Debug] 玩家位置: {playerPos.X},{playerPos.Y},{playerPos.Z}");
                                }
                            }
                        }
                    }
            ...
            isUseKeyBindingRegistered = true;
            Log("[Debug] E键监听已成功注册");
            return true;
        }
        
        // 创建一个临时的传送效果
        private void CreateTeleportEffect(Vector position)
        {
            try
            {
                // 创建一个临时的光效实体(beam_spotlight)
                var effect = Utilities.CreateEntityByName<CBaseEntity>("beam_spotlight");
                if (effect != null)
                {
                    effect.Teleport(position, new QAngle(0, 0, 0), new Vector(0, 0, 0));
                    effect.DispatchSpawn();
            ...
            {
                Log($"[Error] 创建传送效果失败: {ex.Message}");
            }
        }
        
        // 判断玩家是否正在看着某个出生点区域
        private Position? GetSpawnPlayerIsLookingAt(CCSPlayerController player)
        {
            if (player?.PlayerPawn?.Value == null || !player.PlayerPawn.IsValid)
                return null;
                
            // 获取玩家的眼睛位置和视角方向
            // 使用玩家的位置加上眼睛偏移量来获取眼睛位置
            Vector playerOrigin = player.PlayerPawn.Value.CBodyComponent?.SceneNode?.AbsOrigin ?? new Vector(0, 0, 0);
            // 在CS2中，眼睛高度约为64单位
            Vector eyePosition = new Vector(playerOrigin.X, playerOrigin.Y, playerOrigin.Z + 64);
            
            // 获取玩家的视角
            QAngle playerAngles = player.PlayerPawn.Value.EyeAngles;
            Vector forward = QAngleToVector(playerAngles);
            
            // 射线终点 = 眼睛位置 + 方向向量 * 距离
            // 增加射线距离，确保能检测到更远的出生点
            Vector rayEnd = new Vector(
                eyePosition.X + forward.X * 300.0f,
                eyePosition.Y + forward.Y * 300.0f,
                eyePosition.Z + forward.Z * 300.0f
            );
            
            // 用于存储最近的出生点
            Position? closestSpawn = null;
            float closestDistance = float.MaxValue;
            
            // 检查每个出生点区域
            foreach (var spawnEntry in spawnBeamMapping)
            {
                Position spawn = spawnEntry.Value;
                float x = spawn.PlayerPosition.X;
                float y = spawn.PlayerPosition.Y;
                float z = spawn.PlayerPosition.Z;
                
                // 计算出生点中心
                Vector spawnCenter = new Vector(x, y, z);
                
                // 计算射线到出生点中心的最短距离
                float distance = DistanceToRay(spawnCenter, eyePosition, rayEnd);
                
                // 如果距离小于阈值，并且比之前找到的更近，则记录这个出生点
                if (distance < 30.0f && distance < closestDistance)
                {
                    closestDistance = distance;
                    closestSpawn = spawn;
                }
